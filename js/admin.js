(function () {
  ...
  function renderOrders() {
    ...
    rows.innerHTML = db.orders.map(function (o) {
      ...
      return "<tr><td><b>" + esc(o.id) + "</b><small>" + esc(o.date) + "</small></td><td><b>" + esc(o.name) + "</b><small>" + esc(o.phone) + "</small>" + (o.email ? "<small>" + esc(o.email) + "</small>" : "") + "</td><td>" + esc([o.wilaya, o.city].filter(function (x) { return x; }).join(" / ")) + (o.address ? " <small>" + esc(o.address) + "</small>" : "") + couponTag + noteTag + "</td><td><b>" + money(o.total) + "</b>" + fromTag + "<small>" + items + "</small></td><td>" + payCell + "</td><td><select class='status-select status-" + esc(status) + "' data-order-status='" + esc(o.id) + "'>" + orderStatusOptions(status) + "</select></td><td><button class='icon-danger' data-delete-order='" + esc(o.id) + "'>×</button></td></tr>";
    }).join("");
  }
