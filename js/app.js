(function () {
  ...
  function cartQuantity() { return cart.reduce(function (s, x) { return s + x.qty; }, 0); }
  ...
  function updateSettings() {
    var s = db.settings;
    $("brandMark").innerHTML = ElectroDB.getLogo();
    $("#footerMark").innerHTML = ElectroDB.getLogo();
    $("brandName").textContent = s.brand;
    $("#brandSub").textContent = s.brandSubtitle;
    $("#footerSub").textContent = s.brandSubtitle;
    $("#footerText").textContent = s.footerText;
    $("#footerAddress").textContent = s.address;
    $("heroBadge").innerHTML = esc(s.heroBadge) + " <span>✦</span>";
    $("heroTitle").innerHTML = esc(s.heroTitle);
    $("heroText").textContent = s.heroText;
    $("heroCta").childNodes[0].nodeValue = s.heroCta + " ";
    $("heroSecondary").childNodes[0].nodeValue = s.heroSecondary + " ";
    ...
    if (s.announcementEnabled && sessionStorage.getItem("major_announcement_closed") !== "1") {
      $("announcementText").textContent = s.announcement;
      $("announcement").classList.remove("hidden");
    } else $("announcement").classList.add("hidden");
    document.title = s.brand + " — " + (s.brandSubtitle || "");
  }
  ...
  function renderProducts() {
    ...
    grid.innerHTML = list.map(function (p) {
      ...
      return "<article class='product-card' data-product='" + esc(p.id) + "'><div class='product-image' style='--product-color:" + esc(p.color || "#eee") + "'>" + (p.badge ? "<span class='product-badge'>" + esc(p.badge) + "</span>" : "") + "<button class='quick-view' data-view='" + esc(p.id) + "'>↗</button>" + productVisual(p, false) + "</div><div class='product-info'><div class='product-category'>" + esc(categoryName(p.category)) + "</div><h3>" + esc(p.name) + "</h3><p>" + esc(p.description || "") + "</p><div class='rating'><span>★</span> " + Number(p.rating || 0).toFixed(1) + " <small>(" + Number(p.reviews || 0) + ")</small></div><div class='product-bottom'><div><strong>" + money(p.price) + "</strong>" + (p.oldPrice ? "<del>" + money(p.oldPrice) + "</del>" : "") + "</div><button class='add-btn " + (inCart ? "added" : "") + "' data-add='" + esc(p.id) + "'>" + (inCart ? "✓ في السلة" : "＋ أضف للسلة") + "</button></div></div></article>";
    }).join("");
  ...
  function closeOverlays() {
    all(".modal.show").forEach(function (m) { m.classList.remove("show"); });
  ...
  function addToCart(id) {
    ...
    ElectroDB.saveCart(cart); renderCart(); renderProducts(); toast("تمت إضافة "+p.name+" للسلة");
  }
  ...
  function submitOrder(event) {
    ...
    if (!cart.length) return toast("أضف منتجاً أولاً");
    var order = {
      id: "MAJ-" + Date.now().toString().slice(-6),
    ...
    db.orders.unshift(order); persist(); cart = []; ElectroDB.saveCart(cart);
    $("checkoutForm").reset(); closeOverlays(); renderCart(); renderProducts();
    toast("MAJOR STORE استلم طلبك " + order.id);
  }
  ...
  $("announcementClose").onclick = function () { sessionStorage.setItem("major_announcement_closed", "1"); $("announcement").classList.add("hidden"); };
